...
uri = "https://play.google.com/store/apps/details?id=com.nhlife.customer.healthcare&hl=ko-KR&pli=1"
browser.get(uri)
...
print("count comment before done scroll : {}".format(len(element_comment)))             # 돌아가는지 체크용

# 댓글 마지막까지 스크롤 : scrollHeight 확인
# - scrollableDiv.scrollHeight		전체 높이
# - scrollableDiv.scrollTo(0, scrollableDiv.scrollHeight);	현재 높이
...
selector_element = "div.RHo1pe"
element_comment = browser.find_elements(by=By.CSS_SELECTOR, value=selector_element)
print("count comment after done scroll : {}".format(len(element_comment)))             # 돌아가는지 체크용
